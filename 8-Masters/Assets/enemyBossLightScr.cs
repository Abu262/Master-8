...
public class enemyBossLightScr : EnemyScr
{
    bool overheating;
    bool misslesArmed;
    bool misslesLoading;
    ...
    void UpdatePath()
    {
        ...
        float distance1 = Vector3.Distance(transform.position, Human.transform.position);
        ...
        if (Human.GetComponent<PlayerScr>().currentHP <= 0)
        {
            Player = Enemy;
        }
        else if (Enemy.GetComponent<EnemyScr>().currentHP <= 0)
        {
            Player = Human;
        }
        ...
        if (active)
        {
            ...
            if (!dead)
            {
                ...
                if (hit.collider != null)
                {
                    ...
                    //Debug.Log(inMGRange);
                    //Debug.Log(distance);
                    //Debug.Log(shotgunRange);
                    //Debug.Log(Time.time > timeStampL);
                    //Debug.Log(currentAmmoL > 0);
