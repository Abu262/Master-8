...
public class EnemyDesertScr : EnemyScr
{
    ...
    void UpdatePath()
    {
        ...
        if (hit.collider != null && overboosting == false && overboostready)
        {
    ...
    void Update()
    {
        if (active)
        {
            ...
            if (!dead)
            {
                ...
                if (Time.time > timeStampR && currentAmmoR > 0 && hit.collider != null && hit.collider.tag == "Player")
                {
                ...
                if (Time.time > timeStampL && currentAmmoL > 0)
                {
                ...
                if (((currentAmmoL < WlC.ammo) || currentAmmoL <= 0) && Time.time > ammoTimeStampL)
                {
