...
public class EnemyRainForestScr : EnemyScr
{
    ...
    void Start()
    {
        ...
        HC = GM.HeadList[2];
        TC = GM.TorsoList[2];
        WrC = GM.WeaponRList[2];
        WlC = GM.WeaponLList[2];
    ...
    void UpdatePath()
    {
        ...
        {
            Vector3 currentpos = transform.position;
            if (Vector3.Distance(currentpos, Side1.position) > Vector3.Distance(currentpos, Side2.position))
            {
                SideCurrent = Side1;
            }
            else
    ...
    void Update()
    {
        if (active)
        {
            ...
            if (!dead)
            {
                ...
                if (DroneReady && Time.time > timeStampL && currentAmmoL > 0)
                {
                    fireLeft = true;
                ...
                if (carbineArmed && inCannonRange == true && Time.time > timeStampR && currentAmmoR > 0)
                {
                    fireRight = true;
    ...
    void FixedUpdate()
    {
        if (active)
        {
            ...
            if (!dead)
            {
                ...
                Debug.Log(overheating && isBoosting);
