...
public class EnemyScrArena : EnemyScr
{
    bool misslesArmed;
    bool misslesLoading;
    ...
    void Update()
    {
        ...
        if (!dead)
        {
            ...
            if (Time.time > timeStampR && currentAmmoR > 0)
            {
                fireRight = true;
            ...
            if (((fireRight && currentAmmoR < WrC.ammo) || currentAmmoR <= 0) && Time.time > ammoTimeStampR)
            {
            ...
            if (carbineArmed && inCannonRange == true && Time.time > timeStampL && currentAmmoL > 0)
            {
                fireLeft = true;
